...
function openWin($target_id) {
    ...
    if( $( document ).height() > scrollVal + screenHei){
        ...
        document.getElementById($target_id).style.top = scrollVal - screenHei + ( screenHei * 0.1 * 0.2) + "px";
        document.getElementById($target_id).style.display = "block";
    }else{
        addClass(".work_hide", "fixWindow");
        document.getElementById($target_id).style.top = scrollVal - screenHei + "px";
        document.getElementById($target_id).style.display = "block";
    }
}
